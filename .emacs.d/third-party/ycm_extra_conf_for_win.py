...
BASE_FLAGS = [
        ...
        '-I/usr/lib/',
        '-I/usr/include/',
        '-isystem C:\\Program\ Files\\LLVM\\lib\\clang\\5.0.0\\include',
        '-isystem D:\\program\\VisualStudio2015\\VC\\include',
...
def FlagsForInclude2(root, proj_include_path):
    try:
        include_path = FindNearest(root, proj_include_path)
        flags = []
        for dirroot, dirnames, filenames in os.walk(include_path):
            for dir_path in dirnames:
                real_path = os.path.join(dirroot, dir_path)
...
##
##     print("final_flags: {0}".format(final_flags))
##     return {
##             'flags': final_flags,
##             'do_cache': True
##             }
...
def FlagsForFile(filename):
    ...
    if compilation_db_flags:
        ...
        include_flags = FlagsForInclude2(root, 'include')
        if include_flags:
            final_flags = final_flags + include_flags

        include_flags = FlagsForInclude2(root, 'inc')
        if include_flags:
            final_flags = final_flags + include_flags
